...
- `kicad_routing_plugin/ai_backend.py` owns executable discovery, command construction, model/effort choices, skill syntax, JSONL event parsing, final-result extraction, and authentication guidance for the GUI backends.
- The plugin supports Claude Code, OpenAI Codex, and opencode behind one provider-neutral runner. `ai_providers.py` remains a compatibility/test surface for the earlier provider API.
